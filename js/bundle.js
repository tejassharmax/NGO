(function () {
  ...
  function dashboardPage() {
    ...
    if (activities.length === 0) ; else {
      `<div class="activity-list">${activities.map(a => `<div class="activity-item"><span class="activity-icon">${icon(activityIcon(a.type))}</span><div class="activity-copy"><b>${activityLabel(a.type)}</b> for ${a.subject}<time>${timeAgo(a.timestamp)}</time></div></div>`).join('')}</div>`;
    }
    ...
    if (pendingDocs.length === 0) ; else {
      pendingDocs.map((doc, index) => `<div class="list-row"><span class="list-row__avatar">${index + 1}</span><span class="list-row__copy"><b>${doc.docName}</b><span>${doc.childName}</span></span><span class="list-row__meta">Awaiting<br>review</span></div>`).join('');
    }
  ...
  function reportsPage() {
    ...
    children.filter(c => c.status === 'Active' || c.status === 'Verified').length;
    children.filter(c => c.status === 'Pending').length;
    children.filter(c => c.status === 'Verified').length;
