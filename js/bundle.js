(() => {
  ...
  var formatDate = (date) => new Intl.DateTimeFormat("en", { day: "numeric", month: "short", year: "numeric" }).format(new Date(date));
  var pagePath = (page2) => {
  ...
  var statusBadge = (status) => `<span class="badge badge--${status === "Active" || status === "Verified" ? "success" : status === "Pending" ? "warning" : "neutral"}"><i class="badge__dot"></i>${status}</span>`;

  // js/storage.js
  var KEY = "sample-students";
  var ACTIVITY_KEY = "sample-activity";
  var PENDING_KEY = "sample-pending-docs";
  var DOCS_KEY = "sample-uploaded-docs";
  var seedStudents = [];
  ...
  function addUploadedDoc(docName, studentName, fileData, status = "Verified") {
  ...
  function shell(page2, content) {
    ...
    <aside class="sidebar" aria-label="Primary navigation">
      ...
      <nav class="sidebar__nav">${nav.map((item) => navItem(item, page2)).join("")}<div class="sidebar__label">Workspace</div><a class="nav-item ${page2 === "settings" ? "nav-item--active" : ""}" href="${pagePath("settings")}">${icon("settings")}<span class="nav-item__text">Settings</span></a></nav>
  ...
  function studentsPage() {
    ...
    const paginated = students.slice(0, itemsPerPage2);
  ...
  function reportsPage() {
    ...
    const bloodCounts = {};
    students.forEach((s) => {
      const b = (s.blood || "Not specified").trim();
      if (b && b !== "Not added") {
        bloodCounts[b] = (bloodCounts[b] || 0) + 1;
      }
    });
    const sortedBlood = Object.entries(bloodCounts).sort((a, b) => b[1] - a[1]).slice(0, 4);
    const finalBlood = sortedBlood.map(([group, count]) => {
      const pct = total > 0 ? Math.round(count / total * 100) : 0;
      return [group, pct];
    });
    while (finalBlood.length < 4) {
      const defaults = [["O+", 0], ["B+", 0], ["A+", 0], ["AB+", 0]];
      const nextDefault = defaults[finalBlood.length];
      finalBlood.push(nextDefault);
    }
    const recVerifiedPct = total > 0 ? Math.round(verifiedCount / total * 100) : 0;
    const recPendingPct = total > 0 ? Math.round(pendingCount / total * 100) : 0;
    const recActivePct = total > 0 ? Math.max(0, 100 - (recVerifiedPct + recPendingPct)) : 0;
    return shell("reports", `${heading("Reports", "A clear view of admissions, record health, and student demographics.", `<button class="button" type="button" data-report-email>${icon("mail")}Email report</button><button class="button" type="button" data-report-print>${icon("printer")}Print</button><button class="button button--primary" type="button" data-report-export>${icon("download")}Export</button>`)}<section class="card"><div class="filter-row"><label class="field"><span class="field__label">Report period</span><select class="select"><option>Last 6 months</option><option>Last 12 months</option><option>Academic year</option></select></label><label class="field"><span class="field__label">Grade</span><select class="select"><option>All grades</option><option>Grade 6</option><option>Grade 7</option><option>Grade 8</option></select></label><label class="field"><span class="field__label">Status</span><select class="select"><option>All statuses</option><option>Active</option><option>Pending</option></select></label><button class="button button--sm" type="button" data-apply-report>Apply filters</button></div></section><div class="report-grid section-gap"><article class="card report-card"><span class="eyebrow">Admissions</span><div class="report-card__value">${total}</div><p class="report-card__caption">total students in workspace</p></article><article class="card report-card"><span class="eyebrow">Active records</span><div class="report-card__value">${activeCount}</div><p class="report-card__caption">${activePct}% of all student records</p></article><article class="card report-card"><span class="eyebrow">Verified records</span><div class="report-card__value">${verifiedCount}</div><p class="report-card__caption">${pendingCount} records need attention (pending)</p></article></div><div class="dashboard-grid dashboard-grid--lower"><section class="card chart-card"><header class="card__header"><div><h2 class="card__title">Admissions trend</h2><p class="card__caption">New student records created each month</p></div></header><div class="chart-card__body">${admissionsChart()}</div></section><section class="card"><header class="card__header"><div><h2 class="card__title">Gender distribution</h2><p class="card__caption">Across active student records</p></div></header><div class="card__body"><div class="distribution"><div class="distribution__row"><span class="distribution__label">Female</span><div class="progress"><div class="progress__bar" style="width: ${femalePct}%; background: var(--color-violet);"></div></div><span class="distribution__value">${femalePct}%</span></div><div class="distribution__row"><span class="distribution__label">Male</span><div class="progress"><div class="progress__bar" style="width: ${malePct}%; background: var(--color-primary);"></div></div><span class="distribution__value">${malePct}%</span></div><div class="distribution__row"><span class="distribution__label">Other</span><div class="progress"><div class="progress__bar" style="width: ${otherPct}%; background: #94a3b8;"></div></div><span class="distribution__value">${otherPct}%</span></div></div></div></section></div><div class="dashboard-grid dashboard-grid--lower"><section class="card"><header class="card__header"><div><h2 class="card__title">Blood group distribution</h2><p class="card__caption">Useful for medical readiness</p></div></header><div class="card__body"><div class="distribution">${finalBlood.map(([group, value]) => `<div class="distribution__row"><span class="distribution__label">${group}</span><div class="progress"><div class="progress__bar" style="width: ${value}%;"></div></div><span class="distribution__value">${value}%</span></div>`).join("")}</div></div></section><section class="card"><header class="card__header"><div><h2 class="card__title">Record health</h2><p class="card__caption">Completeness and verification state</p></div></header><div class="card__body"><div class="distribution"><div class="distribution__row"><span class="distribution__label">Verified</span><div class="progress"><div class="progress__bar" style="width: ${recVerifiedPct}%; background: var(--color-success);"></div></div><span class="distribution__value">${recVerifiedPct}%</span></div><div class="distribution__row"><span class="distribution__label">In review</span><div class="progress"><div class="progress__bar" style="width: ${recPendingPct}%; background: var(--color-warning);"></div></div><span class="distribution__value">${recPendingPct}%</span></div><div class="distribution__row"><span class="distribution__label">Active / Draft</span><div class="progress"><div class="progress__bar" style="width: ${recActivePct}%; background: var(--color-primary);"></div></div><span class="distribution__value">${recActivePct}%</span></div></div></div></section></div>`);
  }
  function exportPage() {
    const students = getStudents();
    const totalStudents = students.length;
  ...
  function collectStudent(form) {
    ...
    let id = values.id;
    let admission = values.admissionDate || (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
    let status = "Active";
    if (id) {
    ...
    return {
      ...
      class: `${values.grade || "Grade 8"} \xB7 ${values.section || "A"}`,
  ...
  // js/app.js
  var activeSort = { field: "admission", direction: "desc" };
  ...
  if (!isLoggedIn && page !== "login") {
    ...
    document.addEventListener("click", (event) => {
      ...
      if (target.matches("[data-clear-filters]")) {
        document.querySelectorAll("[data-filter-status], [data-filter-grade], [data-filter-blood]").forEach((input) => {
          input.value = "";
        });
        applyTableFilters();
      }
      if (target.matches("[data-delete]")) {
        ...
        modal({ title: `Remove ${student?.name || "student"}?`, body: "This removes the student record from this demo workspace. This action cannot be undone.", confirmText: "Remove student", confirmClass: "button--danger", onConfirm: () => {
          deleteStudent(id);
          applyTableFilters();
          toast("Student removed", "The record has been removed from this workspace.");
        } });
      }
    ...
    document.querySelector("#student-form")?.addEventListener("submit", (event) => {
      ...
      window.setTimeout(() => {
        window.location.href = `${pagePath("student-profile")}?id=${student.id}`;
      }, 500);
    });
