      ...
      ${children.map((c) => {
        ...
        return `
          <button class="sheet-tab ${isActive ? 'is-active' : ''}" type="button" data-report-tab="${c.id}">
            ...
            <span>${escapeHTML(c.name.toUpperCase())}</span>
    ...
    if (growthList && growthList.length > 0) {
      checkupRowsHTML = growthList.map(g => `
        <tr>
          ...
          <td>${escapeHTML(g.complaint || g.symptoms || 'Routine general checkup; healthy')}</td>
          <td><span class="badge badge--neutral">${escapeHTML(g.prescription || g.medication || 'Multivitamins')}</span></td>
          <td><span style="color:#0284c7; font-weight:600;">${escapeHTML(g.eyeCheckup || g.eyeRemarks || '6/6 Normal')}</span></td>
        </tr>
      `).join('');
    } else {
      checkupRowsHTML = `
        <tr>
          ...
          <td>${escapeHTML(c.medicalConditions || 'Routine checkup; baseline vitals recorded')}</td>
          <td><span class="badge badge--neutral">${escapeHTML(c.medications || 'None / Preventive care')}</span></td>
    ...
    if (healthRecList && healthRecList.length > 0) {
      bloodRowsHTML = healthRecList.map(hr => `
        <tr>
          ...
          <td><span class="badge badge--success">${escapeHTML(hr.rbcMorphology || 'Normocytic Normochromic')}</span></td>
          <td><span class="badge badge--neutral">${escapeHTML(hr.wbcMorphology || 'Normal distribution')}</span></td>
          <td><span class="badge badge--success">${escapeHTML(hr.plateletsAdequacy || 'Adequate in smear')}</span></td>
        </tr>
      `).join('');
    } else {
      bloodRowsHTML = `
      ...
      <div class="sheet-container">
        ...
        <div class="sheet-banner-header">
          <div>
            <div style="display:flex; align-items:center; gap:12px; margin-bottom:4px;">
              ...
              <div>
                <div class="sheet-child-title">${escapeHTML(c.name)}</div>
        ...
        <div class="sheet-toolbar-header">
          <div style="font-size:12px; color:var(--color-text-muted); display:flex; align-items:center; gap:8px;">
            ...
            <span>Tab: <b>${escapeHTML(c.name.toUpperCase())}</b> in <i>Student Medical Records</i></span>
          </div>
